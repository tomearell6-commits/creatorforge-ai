    ...
    <Card className="space-y-4">
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          ...
          <select value={s.default_mode} onChange={(e) => setS({ ...s, default_mode: e.target.value })} className="h-10 w-full rounded-lg border border-border bg-background px-3 text-sm">
            <option value="manual">Manual</option><option value="assisted">Assisted</option><option value="full">Full Autopilot</option>
          </select>
        </div>
        <div><Label>Retry limit (failed jobs)</Label><Input type="number" value={s.retry_limit} onChange={(e) => setS({ ...s, retry_limit: Number(e.target.value) })} /></div>
        <div><Label>Low-credit threshold</Label><Input type="number" value={s.low_credit_threshold} onChange={(e) => setS({ ...s, low_credit_threshold: Number(e.target.value) })} /></div>
      </div>

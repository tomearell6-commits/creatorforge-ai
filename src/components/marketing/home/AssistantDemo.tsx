          ...
          <div className="space-y-3">
            {CHAT.map((m, i) => (
              <div key={i} className={`flex gap-2 ${m.from === "user" ? "flex-row-reverse" : ""}`}>
                <span className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full ${m.from === "user" ? "bg-muted text-foreground" : "bg-brand-900 text-brand-300 dark:bg-brand-950/50 dark:text-brand-300"}`}>
